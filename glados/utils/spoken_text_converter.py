...
class SpokenTextConverter:
    ...
    def text_to_spoken(self, text: str) -> str:
        ...
        # 4. Temperature and unit conversions (before whitespace normalization)
        # Handle °F → "degrees", °C → "degrees celsius"
        # System prompt: "Say 'degrees' not 'Fahrenheit'"
        text = re.sub(r"(\d)\s*[°Â°ø]\s*F\b", r"\1 degrees", text)
        text = re.sub(r"(\d)\s*[°Â°ø]\s*C\b", r"\1 degrees celsius", text)
        text = re.sub(r"[°Â°ø]", " degrees ", text)  # standalone degree symbol
        ...
        text = re.sub(r"\bkmh\b", "kilometers per hour", text, flags=re.IGNORECASE)
        ...
        # 6. Convert titles and abbreviations
