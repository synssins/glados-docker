...
class SmartDetectionCameraConfig(BaseModel):
    ...
    entity: str
    """HA camera entity ID (e.g., 'camera.g4_doorbell_high')."""

    speaker: str = "media_player.living_room_2"
...
class SmartDetectionConfig(BaseModel):
    ...
    (e.g., binary_sensor.g4_doorbell_person_detected) are auto-discovered
...
class HomeAssistantSensorJobConfig(BaseModel):
    ...
    def model_post_init(self, __context) -> None:
        try:
            ...
            if not self.ha_ws_url:
                self.ha_ws_url = "ws://192.168.1.104:8123/api/websocket"
            if not self.vision_api_url:
                self.vision_api_url = "http://localhost:8016"
