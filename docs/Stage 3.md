...
- **Container is LAN-only.** `glados.denofsyn.com` is a split-horizon DNS
...
User (webui_chat): "turn off the bedroom lights"
  ...
  → candidates in area=bedroom, domain=light:
    light.bedroom_ceiling, light.bedroom_reading_lamp, light.bedroom_closet
  → naming convention: "lights" (plural, generic) = all light-type
    fixtures in the area → turn off all three
  → resolve_and_execute(
      entity_ids=[light.bedroom_ceiling, light.bedroom_reading_lamp,
...
User (webui_chat): "turn off the bedroom overhead"
  ...
  → resolve_and_execute(
      entity_ids=[light.bedroom_ceiling], service=turn_off)
...
User (api_chat from unknown session): "unlock the front door"
  ...
  → Tier 1: WS conversation/process → resolves lock.front_door
